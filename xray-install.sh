...
checkSystem() {
    if [[ -n $(find /etc -name "redhat-release") ]] || grep "
                exit 1
                ;;
            esac
        fi
...
unInstallSingBox() {
    local type=$1
    if [[ -n "${singBoxConfigPath}" ]]; then
        if grep -q 'tuic' deleted sing-box tuic configuration successfully"
        fi

        if grep -q 'hysteria2' deleted sing-box hysteria2 configuration successfully"
        fi
